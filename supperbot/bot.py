...
from telegram.ext import (
    ...
    CallbackQueryHandler, CommandHandler, ConversationHandler, ChosenInlineResultHandler, InlineQueryHandler,
...
async def help_command(update: Update, context: CallbackContext) -> None:
    ...
    await context.bot.send_message(chat_id=update.effective_chat.id, text="Use /start to use this bot!")
...
async def start_group(update: Update, context: CallbackContext) -> None:
    ...
    await context.bot.send_message(chat_id=update.effective_chat.id, text="Please initialize me in direct messages!")


async def start(update: Update, context: CallbackContext) -> None:
    message = "Welcome to the SupperFarFetch bot! \n\nJust click the buttons below to create a supper jio!"
    keyboard = [
        [InlineKeyboardButton("🆕 Create Supper Jio", callback_data=CallbackType.CREATE_JIO)],
        [InlineKeyboardButton("📖 View Ongoing Jios", callback_data=CallbackType.VIEW_JIOS)]
    ]
...
create_jio_conv_handler = ConversationHandler(
    ...
    states={
        ...
        CallbackType.FINISHED_CREATION: [MessageHandler(filters.TEXT & ~filters.COMMAND, finished_creation)]
    },
    fallbacks=[]
)
...
add_order_conv_handler = ConversationHandler(
    ...
    states={
        CallbackType.CONFIRM_ORDER: [MessageHandler(filters.TEXT & ~filters.COMMAND, confirm_order)]
    },
    fallbacks=[]
)
...
application.add_handler(CommandHandler("start", interested_user, filters.Regex(r'order\d')))
...
unimplemented_callbacks = '|'.join(
    (CallbackType.MODIFY_ORDER, CallbackType.DELETE_ORDER, CallbackType.AMEND_DESCRIPTION, CallbackType.CLOSE_JIO,
     CallbackType.VIEW_JIOS))
application.add_handler(CallbackQueryHandler(not_implemented_callback, unimplemented_callbacks))
